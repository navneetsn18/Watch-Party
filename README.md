...
* **Local HLS transcoding** — uploads are re-encoded to H.264/AAC and chopped into 4-second `.ts` segments by ffmpeg *on your machine*, with a keyframe forced on every segment boundary so seeking is smooth and every segment is clean regardless of the source's own keyframe spacing. Also fixes the classic "MKV plays but has no audio" browser problem. Uses your GPU's hardware encoder (NVENC/QSV/AMF/VideoToolbox) if ffmpeg has one available — commonly 5-10x faster than software encoding — falling back to software automatically if the GPU path fails. Either way, the raw upload is watchable immediately and silently upgrades once HLS is ready.
